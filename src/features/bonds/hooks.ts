...
export function useAllBonds(): IBond[] {
  const bondDepositoryV2 = useBondDepositoryContract()
  const liveBonds = useSingleCallResult(bondDepositoryV2, 'getMarkets', [], undefined, 100_000)
  // console.log(liveBonds?.result?.[0])
  // const loading = liveBonds?.loading

  const rawBondData = liveBonds?.result?.[0]
  console.log(rawBondData)
  const formattedBondData = rawBondData?.map((rawBond) => {
    const formattedBond = formatObjectWithBNFields(rawBond)

    return formattedBond
  })

  return formattedBondData
}
